...
export function useGithubHandler() {
    ...
    const serverCheck = async () => {
        try {
            const response = await fetch("http://13.51.241.63/health-check", {method: 'GET'});
            if (response.ok) {
                return true;
            } else {
                return false;
    ...
    const loginWithGithub = async () => {
        try {
            ...
            if (response) {
                window.location.assign("http://13.51.241.63/github/auth");
            } else {
    ...
    async function logoutGithub() {
        try {
            const response = await serverCheck();
            if (response) {
                const userResponse = await fetch('http://13.51.241.63/github/logout', {
                    method: 'GET',
                    headers: {
    ...
    const getUserData = async () => {
        try {
            const response = await serverCheck();
            if (response) {
                const userResponse = await fetch('http://13.51.241.63/github/user/data', {
                    method: 'GET',
                    headers: {
                    'Content-Type': 'application/json',
                    }, 
    ...
    const getIssuesOfUser = async (user:string) => {
        try {
            const response = await serverCheck();
            if (response) {
                const issues = await fetch('http://13.51.241.63/github/issues/get?user=' + user, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    }, 
    ...
    const closeIssue = async (url:string) => {
        try {
            const response = await serverCheck();
            if (response) {
                const issues = await fetch('http://13.51.241.63/github/issues/close', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    }, 
    ...
    const openIssue = async (url:string) => {
        try {
            const response = await serverCheck();
            if (response) {
                const issues = await fetch('http://13.51.241.63/github/issues/open', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    }, 
    ...
    const updateIssue = async (url:string, title:string, body:string) => {
        try {
            const response = await serverCheck();
            if (response) {
                const issues = await fetch('http://13.51.241.63/github/issues/update', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    }, 
